...
const OurWork = () => {
  ...
  return (
    <div id='our-work' className='flex flex-col items-center gap-7 px-7 sm:px-12 lg:px-24 xl:px-40 pt-30 text-gray-700 dark:text-white'>
       ...
       <div className='grid sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-5xl'>
        {
          workData.map((work, index) => (
            <div key={index} className='hover:scale-102 duration-500 transition-all cursor-pointer'>
              ...
              <p className='text-sm opicity-60 w-5/6'>{work.description}</p>
           </div>
          ))
        }
       </div>
